;(function($) {
    ...
    */
    ...
    $.widget("ui.dropdownchecklist", {
        ...
        _createDropItem: function(index, value, text, checked, hidden) {
            var self = this, singleSelection = self.singleSelection;
            ...
            // -Add Start CK_2014_0002
            checkBox.attr("id", this.element[0].id + "Chk" + index);
            checkBox.attr("name", this.element[0].id + "Chk");
            // -Add End CK_2014_0002
            ...
            checkBox.click(function(event) {
                ...
                self.sourceSelect.trigger('changed'); // -added by HISYS
            });
            // check/uncheck the item on clicks on the entire item div
            var checkItem = function(event) {
                event.stopPropagation();
                // -replaced by HISYS Start
                self.sourceSelect.trigger('changed'); 
                if (singleSelection) {
                    checkBox.attr("checked", true);
                    self.dropWrapper.drop = true;
                    self._toggleDropContainer();  
                } else {
                    var checked = checkBox.attr("checked");
                    checkBox.attr("checked", !checked)
                }
                // -replaced by HISYS End
        ...
        _syncSelected: function(senderCheckbox) {
            ...
            allCheckboxes.each(function(index) {
                ...
                if (self.singleSelection && senderCheckbox.attr("index") == index) {
                    isChecked = true;
                ...
                $(selectOptions[index]).attr("selected", isChecked);
            });
        ...
        _updateControlText: function() {
            ...
            var singleSelection = self.singleSelection;
        ...
        _init: function() {
            ...
            //sourceSelect.attr("multiple", "multiple"); // -deleted by HISYS
            self.sourceSelect = sourceSelect;
            self.singleSelection = !sourceSelect.attr("multiple"); // -added by HISYS
            
            // -added by HISYS
            if(options.initWithNoSelection) {
            	var selectOptions = self.sourceSelect.children('option:selected');
            	selectOptions.each(function() {
        ...
        // -added by HISYS
        // upadte the check state of checkboxes
        refresh: function() {
        	var self = this, options = this.options, sourceSelect = this.sourceSelect, controlWrapper = this.controlWrapper, dropWrapper = this.dropWrapper;
        	// update the items from the source select element
            var dropCalculatedSize = self._appendItems();

            var selectOptions = sourceSelect.get(0).options;
        	var allCheckboxes = dropWrapper.find("input");
        	if (options.firstItemChecksAll) {
        		allCheckboxes.attr('checked', Boolean($(selectOptions[0]).attr('selected')));
			} else {
			    for(var i = 0; i < selectOptions.length; i++) {
			        var isSelected = false;
                    if ($(selectOptions[i]).attr('selected') || $(selectOptions[i]).attr('selected') === 'selected') {
                        isSelected = true;
                    }
                    $(allCheckboxes[i]).attr('checked', isSelected);
			    }
			}
    ...
    $.extend($.ui.dropdownchecklist, {
        defaults: {
            ...
            minWidth: 50,
            initWithNoSelection: false // -added by HISYS
        }
    });
