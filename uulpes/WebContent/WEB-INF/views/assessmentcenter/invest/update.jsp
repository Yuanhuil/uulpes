...
<form:form id="investform" action="${baseaction}/save.do" method="post" modelAttribute="invest">
    ...
    <div class="buttonContent" id="invest_query">
        <ul>
            <li>
                ...
                <input class="button-small blue" type="button" value="保存问卷" id="invest_sav"
                    chref="${baseaction}/save.do">
            </li>
            <li>
    ...
    <div class="survey"
        style="overflow: auto; padding-top: 15px; padding-right: 24px; padding-bottom: 15px; padding-left: 25px; background-image: none; background-attachment: scroll; background-repeat: repeat; background-position-x: 0%; background-position-y: 0%; background-color: rgb(255, 255, 255);">
        <div style="width: 780px; margin: 0 auto; padding-left: 10px;">
            <div id="question" class="surveycontent">
                <div id="divId" class="surveyhead"
                    style="border: 2px solid #ffffff; margin-top: 3px; width: 730px; cursor: pointer;"
                    title="鼠标点击编辑问卷标题与问卷说明" onclick="showDlg();">
                    <h1 id="investtitle" style="cursor: pointer;"></h1>
                    <div id="investdescn" class="surveydescription" style="cursor: pointer;"></div>
                </div>
            </div>
            <div style="clear: both;"></div>
        </div>
        <div id="divbatchq" style="display: none; margin: 20px auto 10px; width: 220px;">
            <a href="javascript:" class="BS_btn"
                style="height: 35px; line-height: 35px; width: 200px; text-align: center; background: #56bbed;">批量添加题目</a>
        </div>
...
<div id="divBox" class="hideDlg" style="">
    <ul class="div_title_attr_question" style="padding: 10px 0 0 10px; margin: 0px; background: #fff;">
        <li style="list-style-type: none;">
            <strong>问卷标题：</strong>
            <input id="paper_attr_title" title="在此输入问卷的标题，例如：老师授课满意度调查" style="font-size: 12px; width: 400px;" class=""
                type="text" size="54" maxlength="100" />
        </li>
        <li style="margin-top: 15px;">
            <table>
                <tr>
                    ...
                    <td>
                        <textarea id="paper_attr_desc" name="content" onblur="paper_attr_desc_onblur(this);" rows="10"
                            title="在此输入问卷说明"></textarea>
                        <br />
                    </td>
                </tr>
            </table>
    ...
    <div style="text-align: center; margin-top: 10px;">
        <a href="javascript:;" onclick="update_title()" class="sumitbutton">确定</a> <a href="javascript:;"
            class="sumitbutton" onclick="cancel();">取消</a>
    </div>
</div>
<script type="text/javascript">
    function update_title() {
        $("#investtitle").text($("#paper_attr_title").val());
        $("#investdescn").html($("#paper_attr_desc").val());
        cancel();
    };
    $(function() {
        debugger;
        ...
        debugger;
        var ttt = $.trim(sobj.title);
        var title = sobj.title.replace(/&nbsp;/g, "");
        $("#investtitle").text($.trim(title));
        $("#paper_attr_title").val($.trim(title));
        //var sss = sobj.descn.replace(/[<>&"]/g,function(c){return {'<':'&lt;','>':'&gt;','&':'&amp;','"':'&quot;'}[c];});
        //layer.open({content:sss});
        $("#investdescn").html(sobj.descn);
        $("#paper_attr_desc").html(sobj.descn);
