...
<body>
    ...
    <div class="information">
        <ul class='report-ul'>
            <img src='${page.userinfo.zp}' alt=""
                style="max-width: 200px; max-height: 150px; width: 200px; height: 150px; margin-top: 20px;" />
            ...
            <li class='report-tail'>工号：${page.userinfo.gh }</li>
        </ul>
    </div>
    <div class="report-title">
        <ul class='report-ul'>
    ...
    <div>
        <c:choose>
            ...
            <c:otherwise>
                <div class="table01">${page.dimScoreTable}</div>
                <div class="chart">
                    <c:if test="${not empty page.isImage1Display && page.isImage1Display==1}">
                        <c:if test="${not empty page.image1}">
                            <div>
                                <img src='${baseaction}/scalechart.do?${page.image1}' />
                            </div>
                        </c:if>
                    </c:if>
                    <c:if test="${not empty page.image2}">
                        <div>
                            <img src='${baseaction}/reportchart.do?${page.image2}' />
                        </div>
                    </c:if>
                    <c:if test="${not empty page.image3}">
                        <div>
                            <img src='${baseaction}/reportchart.do?${page.image3}' />
                        </div>
                    </c:if>
                </div>
            </c:otherwise>
    ...
    <div class='evaluation'>
        <c:if test="${page.scaleshortname!='MBTI'}">
            <c:forEach var="dimDetail" items="${page.dimDetailList}">
                <h3>${dimDetail.dimtitle}</h3>
                <c:if test="${not empty dimDetail.desc}">
                    <p>结果解释：${dimDetail.desc}</p>
                </c:if>
                <c:if test="${not empty dimDetail.device}">
                    <p>指导建议：${dimDetail.device}</p>
                </c:if>
                <c:if test="${!empty dimDetail.subdimlist}">
                    <c:forEach var="subDimDetail" items="${dimDetail.subdimlist}">
                        <h4>${subDimDetail.dimtitle}</h4>
                        <c:if test="${not empty subDimDetail.desc}">
                            <p>结果解释：${subDimDetail.desc}</p>
                        </c:if>
                        <c:if test="${not empty subDimDetail.device}">
                            <p>指导建议：${subDimDetail.device}</p>
                        </c:if>
                    </c:forEach>
                </c:if>
                <c:if test="${not empty dimDetail.image}">
                    <div class="chart">
                        <img src='${baseaction}/${dimDetail.image}' />
                    </div>
                </c:if>
            </c:forEach>
            <c:if test="${not empty page.summarizedesc}">
                <h4>总结果解释：${page.summarizedesc}</h4>
            </c:if>
            <c:if test="${not empty page.summarizedevice}">
                <h4>总指导建议：${page.summarizedevice}</h4>
            </c:if>
        </c:if>
        <c:if test="${page.scaleshortname=='MBTI'}">
            <c:if test="${not empty page.mbtiDesripter}">
                <p>结果解释：${page.mbtiDesripter}</p>
            </c:if>
            <c:if test="${not empty page.mbtiAdvice}">
                <p>指导建议：${page.mbtiAdvice}</p>
            </c:if>
        </c:if>
    </div>
    <div class='report-title'>
