            ...
            <tr>
                ...
                <td>
                    <c:if test="${m.fid !=0}">
                        ...
                        <shiro:hasPermission name="consultcenter:centersetting:delete">
                            <input class="button-small white del" id="${m.id}" type="button" value="删除">
                        </shiro:hasPermission>
                    </c:if>
                    <input class="button-small white switch" id="${m.id}" type="button" value="切换">
                </td>
            </tr>
        </c:forEach>
    </tbody>
...
<script type="text/javascript">
    ...
    jQuery('.del').bind('click', function() {
        if (confirm('确认删除')) {
            ...
            divLoadUrl("consultTypeList", '/pes/consultcenter/consultType/delete.do?id=' + id);
        }
    });
    jQuery('.switch').bind('click', function() {
        var id = jQuery(this).attr("id");
        divLoadUrl("consultTypeList", '/pes/consultcenter/consultType/updateStatus.do?id=' + id);
    });
</script>