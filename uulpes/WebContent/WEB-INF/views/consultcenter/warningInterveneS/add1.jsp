...
<div id="dialog-form1" title="编辑">
	<form:form id="editForm"
		action="/pes/consultcenter/warningInterveneS/saveAdd.do"
	...
	<c:if test="${fn:length(xdlist) == 1 }">
		...
		<div class="filterContent">
			...
			<li><label class="name04">处理角色</label> <select id="roleId" class="input_160"  onChange="selectTeacher()">
					<option value="">----</option>					
					<option value="21">心理咨询员</option>
					</select>
			...
			<li><label class="name04">处理者</label> <form:select path="disposePerson" class="input_160">
				...
				<option value="${warningInterveneS.disposePerson}" >${warningInterveneS.disposePerson}</option>
				</form:select>
			</li> 
			</ul>
		</div>
...
//yearRange:'2010:2012',	//显示可供选择的年份
	...
	$("textarea#record").ckeditor({height: '240px'});
