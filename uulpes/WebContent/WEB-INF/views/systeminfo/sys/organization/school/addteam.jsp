...
<c:choose>
	<c:when test="${op eq '新增'}">
		...
		<c:set var="formaction" scope="page" value="${ctx}/systeminfo/sys/organization/school/${team.schoolid }/team/create.do"/>
	</c:when>
	...
	<c:when test="${op eq '修改'}">
		<c:remove var="formaction"/>
		<c:set var="formaction" scope="page" value="${ctx}/systeminfo/sys/organization/school/team/${team.id }/${team.schoolid }/update.do"/>
	</c:when>
	<c:otherwise>
		<c:remove var="formaction"/>
		<c:set var="formaction" scope="page" value="${ctx}/systeminfo/sys/organization/school/${team.schoolid }/team/create.do"/>
...
<div id="dialog-form" title="${empty op ? '新增' : op}心理队伍">
  <form:form id="editteamForm" method="post" commandName="team"
   action="${formaction}" >
