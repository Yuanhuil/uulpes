...
public class TeacherGroupReportForCounty {
    ...
    public TeacherGroupReportForCounty(String countyname, String countyid, String scaleid, int roleid, String rolename,
            String[] schoolids, String starttime, String endtime) {
        ...
        // this.xxmcArray = schoolnames;
        xxmcArray = new String[schoolids.length];
        ...
        OrganizationMapper organizationMapper = (OrganizationMapper) SpringContextHolder.getBean("organizationMapper",
                OrganizationMapper.class);
        for (int i = 0; i < schoolids.length; i++) {
            // String xxmc =schoolnames[i];
            long orgid = Long.parseLong(schoolids[i]);
            Organization org = organizationMapper.selectByPrimaryKey(orgid);
            String xxmc = org.getName();
            // xxmcArray[i] = xxmc;
            xxmcArray[i] = schoolids[i];
            Map<?, ?> param = UtilMisc.toMap("countyid", countyid, "orgid", orgid, "scaleid", scaleid, "roleid", roleid,
                    "rolename", rolename, "starttime", starttime, "endtime", endtime);
            List<Map> teaCountMapList = examResultMapper.queryTeaGroupStaticForCounty(param);
            WholeStat wholeStat = new WholeStat();
            for (Map countMap : teaCountMapList) {
            ...
            // wholeStatMap.put(xxmc, wholeStat);
            wholeStatMap.put(schoolids[i], wholeStat);
            schoolMap.put(schoolids[i], xxmc);
        }
    }

    public void incExmresult(ExamResult examResult) {
        if (currentUserId != examResult.getUserId()) {
            // String xxmc = examResult.getXxmc();
            long orgid = examResult.getUserOrgId();
            // WholeStat wholeStat = wholeStatMap.get(xxmc);
            WholeStat wholeStat = wholeStatMap.get(String.valueOf(orgid));
    ...
    public String buildWholeStaticTable() {
        HtmlTable table = new HtmlTable("表1" + countyname + rolename + "测评对象统计量");
        ...
        WholeStat wholeStat = wholeStatMap.get(rolename);
    ...
    public String buildWholeStaticTable1() {
        ...
        for (int i = 0; i < this.xxmcArray.length + 1; i++) {
            List<Object> values = null;
            if (i == xxmcArray.length) {
            ...
            String xxmc = schoolMap.get(xxmcArray[i]);
            TreeNode tn0 = new TreeNode(xxmc);
            // TreeNode tn0 = new TreeNode(xxmcArray[i]);
            TreeNode tn1 = new TreeNode("男");
            TreeNode tn2 = new TreeNode("女");
            table.addNode(tn0);
            tn0.addChild(tn1);
            ...
            WholeStat wholeStat = wholeStatMap.get(xxmcArray[i]);
            values = new ArrayList<Object>();
    ...
    public String buildRootDimWarningStaticTable() {
        ...
        if (scale.isWarningOrNot())
            table.setCaption("表2 " + countyname + rolename + "学生测评预警结果统计");
        else
            table.setCaption("表2 " + countyname + rolename + "学生测评得分结果统计");
        ...
        if (dim != null) {
            ...
            temp = MathUtils.round(temp, 2);
            dimStat.setWnp1(temp);
            // sb.append("一级预警为为").append(wn1).append("人，占总样本的").append("XX%").append("；其中男教师").append("").append("名，占总数据的").append("").append("；女教师").append("").append("名，占总数据的").append("").append("%。");
    ...
    private List<String> buildDescripterWithW0() {
        ...
        if (hasTotl()) {
            ...
            if (scale.isWarningOrNot()) {
                ...
                sb.append("三级预警为").append(dimStat.getWn3()).append("人，占总样本的").append(dimStat.getWnp3())
                        .append("%；其中男教师").append(dimStat.getWnm3()).append("名，占总数据的").append(dimStat.getWnmp3())
                        .append("%；女教师").append(dimStat.getWnw3()).append("名，占总数据的").append(dimStat.getWnwp3())
                        .append("%。");
                descListW0.add(sb.toString());
                sb = new StringBuilder();
                sb.append("二级预警为").append(dimStat.getWn2()).append("人，占总样本的").append(dimStat.getWnp2())
                        .append("%；其中男教师").append(dimStat.getWnm2()).append("名，占总数据的").append(dimStat.getWnmp2())
                        .append("%；女教师").append(dimStat.getWnw2()).append("名，占总数据的").append(dimStat.getWnwp2())
                        .append("%。");
                descListW0.add(sb.toString());
                sb = new StringBuilder();
                sb.append("一级预警为").append(dimStat.getWn1()).append("人，占总样本的").append(dimStat.getWnp1())
                        .append("%；其中男教师").append(dimStat.getWnm1()).append("名，占总数据的").append(dimStat.getWnmp1())
                        .append("%；女教师").append(dimStat.getWnw1()).append("名，占总数据的").append(dimStat.getWnwp1())
                        .append("%。");
                descListW0.add(sb.toString());
            } else {
                sb = new StringBuilder();
                sb.append("得分等级为五级的有").append(dimStat.getWn5()).append("人，占总样本的").append(dimStat.getWnp5())
                        .append("%；其中男教师").append(dimStat.getWnm5()).append("名，占总数据的").append(dimStat.getWnmp5())
                        .append("%；女教师").append(dimStat.getWnw5()).append("名，占总数据的").append(dimStat.getWnwp5())
                        .append("%。");
                descListW0.add(sb.toString());
                sb = new StringBuilder();
                sb.append("得分等级为四级的有").append(dimStat.getWn4()).append("人，占总样本的").append(dimStat.getWnp4())
                        .append("%；其中男教师").append(dimStat.getWnm4()).append("名，占总数据的").append(dimStat.getWnmp4())
                        .append("%；女教师").append(dimStat.getWnw4()).append("名，占总数据的").append(dimStat.getWnwp4())
                        .append("%。");
                descListW0.add(sb.toString());
                sb = new StringBuilder();
        ...
        for (int i = 0; i < dimStatList.size(); i++) {
            ...
            if (level == 1) {
                if (i == 0) {
                    ...
                    sb2.append("人，占总样本的");
                    sb2.append(kv.getValue().getWnp1()).append("%;其中男教师").append(kv.getValue().getWnm1())
                            .append("名，占总数据的");
                    sb2.append(kv.getValue().getWnmp1()).append("%;");
                    sb2.append("女教师").append(kv.getValue().getWnw1()).append("名，占总数据的");
            ...
            if (level == 2) {
                if (i == 0) {
                    ...
                    sb2.append("人，占总样本的");
                    sb2.append(kv.getValue().getWnp2()).append("%;其中男教师").append(kv.getValue().getWnm2())
                            .append("名，占总数据的");
                    ...
                    sb2.append(kv.getValue().getWnp2()).append("%;其中男教师").append(kv.getValue().getWnm2())
                            .append("名，占总数据的");
                    sb2.append(kv.getValue().getWnmp2()).append("%;");
                    sb2.append("女教师").append(kv.getValue().getWnw2()).append("名，占总数据的");
                    sb2.append(kv.getValue().getWnwp2()).append("%。");
                } else {
                    sb1.append("维度").append(kv.getKey()).append("、");
                    sb2.append("然后依次是维度").append(kv.getKey()).append("，检出数为").append(kv.getValue().getWn2());
            ...
            if (level == 3) {
                if (i == 0) {
                    ...
                    sb2.append("人，占总样本的");
                    sb2.append(kv.getValue().getWnp3()).append("%;其中男教师").append(kv.getValue().getWnm3())
                            .append("名，占总数据的");
                    sb2.append(kv.getValue().getWnmp3()).append("%;");
                    sb2.append("女教师").append(kv.getValue().getWnw3()).append("名，占总数据的");
            ...
            if (level == 4) {
                if (i == 0) {
                    ...
                    sb2.append("人，占总样本的");
                    sb2.append(kv.getValue().getWnp4()).append("%;其中男教师").append(kv.getValue().getWnm4())
                            .append("名，占总数据的");
                    sb2.append(kv.getValue().getWnmp4()).append("%;");
                    sb2.append("女教师").append(kv.getValue().getWnw4()).append("名，占总数据的");
            ...
            if (level == 5) {
                if (i == 0) {
                    ...
                    sb2.append("人，占总样本的");
                    sb2.append(kv.getValue().getWnp5()).append("%;其中男教师").append(kv.getValue().getWnm5())
                            .append("名，占总数据的");
                    sb2.append(kv.getValue().getWnmp5()).append("%;");
                    sb2.append("女教师").append(kv.getValue().getWnw5()).append("名，占总数据的");
