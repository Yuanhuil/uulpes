...
import com.njpes.www.service.baseinfo.organization.OrganizationServiceImpl;
...
public class TeacherGroupReportForSubSchool {
    ...
    private String[] zsxxorgidArray;
    private String[] zsxxmcArray;
    ...
    public TeacherGroupReportForSubSchool(String cityname, String[] zsxxorgids, String scaleid, int roleid,
            String rolename, String starttime, String endtime) {
        this.zsxxorgidArray = zsxxorgids;
        ...
        zsxxmcArray = new String[zsxxorgids.length];
        // 以下计算总样本数
        OrganizationServiceI organizationService = (OrganizationServiceI) SpringContextHolder
                .getBean("organizationService", OrganizationServiceImpl.class);
        ExamResultMapper examResultMapper = (ExamResultMapper) SpringContextHolder.getBean("examResultMapper",
                ExamResultMapper.class);
        for (int i = 0; i < zsxxorgids.length; i++) {
            String xxorgid = zsxxorgids[i];
            Organization orgniaztion = organizationService.selectOrganizationById((long) Integer.parseInt(xxorgid));
            zsxxmcArray[i] = orgniaztion.getName();
            Map<?, ?> param = UtilMisc.toMap("orgid", xxorgid, "scaleid", scaleid, "roleid", roleid, "rolename",
                    rolename, "starttime", starttime, "endtime", endtime);
            List<Map> stuCountMapList = examResultMapper.queryTeaGroupStaticForCitySubSchool(param);
            ...
            wholeStatMap.put(orgniaztion.getName(), wholeStat);
        }
    }
    ...
    public void incExmresult(ExamResult examResult) {
        if (currentUserId != examResult.getUserId()) {
            ...
            String xxmc = examResult.getBjmc();
    ...
    public String buildWholeStaticTable() {
        HtmlTable table = new HtmlTable("表1 直属学校" + rolename + "测评对象统计量");
    ...
    public String buildWholeStaticTable1() {
        ...
        for (int i = 0; i < this.zsxxmcArray.length + 1; i++) {
            List<Object> values = null;
            if (i == zsxxmcArray.length) {
            ...
            TreeNode tn0 = new TreeNode(zsxxmcArray[i]);
            TreeNode tn1 = new TreeNode("男");
            TreeNode tn2 = new TreeNode("女");
            table.addNode(tn0);
            tn0.addChild(tn1);
            ...
            WholeStat wholeStat = wholeStatMap.get(zsxxmcArray[i]);
            values = new ArrayList<Object>();
    ...
    public String buildRootDimWarningStaticTable() {
        ...
        if (scale.isWarningOrNot())
            table.setCaption("表2 " + countyname + rolename + "测评预警结果统计");
        else
            table.setCaption("表2 " + countyname + rolename + "测评得分结果统计");
