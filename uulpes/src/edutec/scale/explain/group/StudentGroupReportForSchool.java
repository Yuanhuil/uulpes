...
public class StudentGroupReportForSchool {
    ...
    private String[] classTitleArray;
    ...
    public StudentGroupReportForSchool(long orgid, String scaleid, String njmc, String[] classGroup, String starttime,
            String endtime) {
        ...
        // wholeStat = new WholeStat();
        // gradeorderid = Integer.parseInt(gradeOrderId);
        // 以下计算总样本数
        this.njmc = njmc;
        this.bjidArray = new String[classGroup.length];
        classTitleArray = new String[classGroup.length];
        ExamResultMapper examResultMapper = (ExamResultMapper) SpringContextHolder.getBean("examResultMapper",
                ExamResultMapper.class);
        ClassSchoolMapper classSchoolMapper = (ClassSchoolMapper) SpringContextHolder.getBean("classSchoolMapper",
                ClassSchoolMapper.class);
        for (int i = 0; i < classGroup.length; i++) {
            // String bjmc =classGroup[i];
            Long bjid = Long.parseLong(classGroup[i]);
            bjidArray[i] = classGroup[i];
            ClassSchool cs = classSchoolMapper.selectByPrimaryKey(bjid);
            String bjmc = cs.getBjmc();
            classTitleArray[i] = bjmc;
            // bjmcArray[i] = bjmc;
            Map<?, ?> param = UtilMisc.toMap("orgid", orgid, "scaleid", scaleid, "njmc", njmc, "bj", bjid, "starttime",
                    starttime, "endtime", endtime);
            List<Map> stuCountMapList = examResultMapper.queryStuGroupStaticForSchool(param);
            ...
            wholeStatMap.put(classGroup[i], wholeStat);
            classMap.put(classGroup[i], bjmc);
        }
        // dimTitles = new ArrayList<String>(scale.getDimensionSize());

        // initItems( gradeorderid, classGroup);
    }
    ...
    public void incExmresult(ExamResult examResult) {
        if (currentUserId != examResult.getUserId()) {
            ...
            String bjmc = examResult.getBjmc();
            long bjid = examResult.getUserClassId();
            // WholeStat wholeStat = wholeStatMap.get(classid+"");
            WholeStat wholeStat = wholeStatMap.get(String.valueOf(bjid));
    ...
    public String buildWholeStaticTable() {
        HtmlTable table = new HtmlTable("表1" + njmc + "/" + classTitleArray[0] + "测评对象统计量");
        ...
        WholeStat wholeStat = wholeStatMap.get(String.valueOf(bjidArray[0]));
        List<Object> values = new ArrayList<Object>();

        values.add(wholeStat.getnMaleTotal() + "");
        double temp = wholeStat.getnTotal() == 0 ? 0
                : ((float) wholeStat.getnMaleTotal() / wholeStat.getnTotal()) * 100;
        temp = MathUtils.round(temp, 2);
        values.add(temp + "");
        values.add(wholeStat.getnValidMale() + "");
        temp = wholeStat.getnTotal() == 0 ? 0 : ((float) wholeStat.getnValidMale() / wholeStat.getnTotal()) * 100;
        temp = MathUtils.round(temp, 2);
        values.add(temp + "");
        ...
        temp = wholeStat.getnTotal() == 0 ? 0 : ((float) wholeStat.getnFemaleTotal() / wholeStat.getnTotal()) * 100;
        temp = MathUtils.round(temp, 2);
        values.add(temp + "");
        values.add(wholeStat.getnValidFemale() + "");
        temp = wholeStat.getnTotal() == 0 ? 0 : ((float) wholeStat.getnValidFemale() / wholeStat.getnTotal()) * 100;
        temp = MathUtils.round(temp, 2);
        values.add(temp + "");
        ...
        temp = wholeStat.getnTotal() == 0 ? 0 : ((float) wholeStat.getnTotal() / wholeStat.getnTotal()) * 100;
        temp = MathUtils.round(temp, 2);
        values.add(temp + "");
        values.add(wholeStat.getnValid() + "");
        temp = wholeStat.getnTotal() == 0 ? 0 : ((float) wholeStat.getnValid() / wholeStat.getnTotal()) * 100;
        temp = MathUtils.round(temp, 2);
        values.add(temp + "");
    ...
    public String buildWholeStaticTable1() {
        HtmlTable table = new HtmlTable("表1" + njmc + "测评对象统计量");
        // table.setTableStyle("width='90%' border='0' cellspacing='1' " +
        // "cellpadding='0' class='table_style'");
        ...
        for (int i = 0; i < bjidArray.length + 1; i++) {
            List<Object> values = null;
            if (i == bjidArray.length) {
            ...
            String bjmc = classMap.get(bjidArray[i] + "");
            TreeNode tn0 = new TreeNode(njmc + "/" + bjmc);
            TreeNode tn1 = new TreeNode("男");
            TreeNode tn2 = new TreeNode("女");
            table.addNode(tn0);
            tn0.addChild(tn1);
            ...
            WholeStat wholeStat = wholeStatMap.get(String.valueOf(bjidArray[i]));
            values = new ArrayList<Object>();

            values.add(wholeStat.getnMaleTotal() + "");
            totalMale += wholeStat.getnMaleTotal();
            double temp = wholeStat.getnTotal() == 0 ? 0
                    : wholeStat.getnTotal() == 0 ? 0
                            : ((float) wholeStat.getnMaleTotal() / wholeStat.getnTotal()) * 100;
            temp = MathUtils.round(temp, 2);
            values.add(temp + "");
            values.add(wholeStat.getnValidMale() + "");
            totalValidMale += wholeStat.getnValidMale();
            temp = wholeStat.getnTotal() == 0 ? 0
                    : wholeStat.getnTotal() == 0 ? 0
                            : ((float) wholeStat.getnValidMale() / wholeStat.getnTotal()) * 100;
            temp = MathUtils.round(temp, 2);
            values.add(temp + "");
            tn1.setValues(values);
            ...
            temp = wholeStat.getnTotal() == 0 ? 0
                    : wholeStat.getnTotal() == 0 ? 0
                            : ((float) wholeStat.getnFemaleTotal() / wholeStat.getnTotal()) * 100;
            temp = MathUtils.round(temp, 2);
            values.add(temp + "");
            values.add(wholeStat.getnValidFemale() + "");
            ...
            temp = wholeStat.getnTotal() == 0 ? 0
                    : wholeStat.getnTotal() == 0 ? 0
                            : ((float) wholeStat.getnValidFemale() / wholeStat.getnTotal()) * 100;
            temp = MathUtils.round(temp, 2);
            values.add(temp + "");
            tn2.setValues(values);
    ...
    public String buildRootDimWarningStaticTable() {
        ...
        if (this.bjidArray.length < 2) {
            if (scale.isWarningOrNot())
                table.setCaption("表2 " + njmc + classTitleArray[0] + "学生测评预警结果统计");
            else
                table.setCaption("表2 " + njmc + classTitleArray[0] + "学生测评得分结果统计");
        } else {
            if (scale.isWarningOrNot())
                table.setCaption("表2 " + njmc + "学生测评预警结果统计");
            else
                table.setCaption("表2 " + njmc + "学生测评得分结果统计");
        }
        // table.setTableStyle("width='90%' border='0' cellspacing='1' " +
        // "cellpadding='0' class='table_style'");
        // table.setTableStyle("width='500px' class='statictablestyle'");
        ...
        for (Dimension dim : scale.getDimensions()) {
            if (dim.isRoot()) {
                if (!Dimension.SUM_SCORE_DIM.equals(dim.getId())) {
                    ...
                    double temp = totalNum == 0 ? 0 : ((float) dimStat.getWnm1() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWnw1() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWn1() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWnm2() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    dimStat.setWnmp1(temp);
                    tempNMale += ws.getMaleNum();
                    tempNMaleP += temp;
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWnw2() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWn2() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWnm3() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWnw3() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    temp = totalNum == 0 ? 0 : ((float) dimStat.getWn3() / totalNum) * 100;
                    temp = MathUtils.round(temp, 2);
                    ...
                    if (!scale.isWarningOrNot()) {
                        ...
                        temp = totalNum == 0 ? 0 : ((float) dimStat.getWnm4() / totalNum) * 100;
                        temp = MathUtils.round(temp, 2);
                        ...
                        temp = totalNum == 0 ? 0 : ((float) dimStat.getWnw4() / totalNum) * 100;
                        temp = MathUtils.round(temp, 2);
                        ...
                        temp = totalNum == 0 ? 0 : ((float) dimStat.getWn4() / totalNum) * 100;
                        temp = MathUtils.round(temp, 2);
                        ...
                        temp = totalNum == 0 ? 0 : ((float) dimStat.getWnm5() / totalNum) * 100;
                        temp = MathUtils.round(temp, 2);
                        ...
                        temp = totalNum == 0 ? 0 : ((float) dimStat.getWnw5() / totalNum) * 100;
                        temp = MathUtils.round(temp, 2);
                        ...
                        temp = totalNum == 0 ? 0 : ((float) dimStat.getWn5() / totalNum) * 100;
                        temp = MathUtils.round(temp, 2);
    ...
    public String buildSubDimWarningStatTable(String id) {
        ...
        for (Dimension subDim : dimList) {
            ...
            values.add(ws.getFemaleNum());
            tempNFemale += ws.getFemaleNum();
            temp = totalNum == 0 ? 0 : ((float) ws.getFemaleNum() / totalNum) * 100;
            temp = MathUtils.round(temp, 2);
            tempNFemaleP += temp;
            values1.add(temp);
            dimStat.setWn1(dimStat.getWnm1() + dimStat.getWnw1());
            ...
            temp = MathUtils.round(temp, 2);
            dimStat.setWnmp1(temp);
            temp = totalNum == 0 ? 0 : ((float) ws.getMaleNum() / totalNum) * 100;
            temp = MathUtils.round(temp, 2);
            values1.add(temp);
            tempNMale += ws.getMaleNum();
            tempNMaleP += temp;
