...
public class SetCommonDataInterceptor extends HandlerInterceptorAdapter {
    ...
    private static final String[] DEFAULT_EXCLUDE_PARAMETER_PATTERN = new String[] { "\\&\\w*page.pn=\\d+",
            "\\?\\w*page.pn=\\d+", "\\&\\w*page.size=\\d+" };
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if (request.getSession().getAttribute(Constants.CURRENT_USER) != null
                && request.getAttribute(Constants.ORG_ID_PROP) == null) {
            Account currentUser = (Account) request.getSession().getAttribute(Constants.CURRENT_USER);
            ...
            if (organizationJobs == null || organizationJobs.size() == 0) {
                ...
                orgid = organizationJobs.get(0).getOrgId();
            request.setAttribute(Constants.ORG_ID_PROP, orgid);
        }
