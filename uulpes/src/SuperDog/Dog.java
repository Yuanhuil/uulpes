...
public class Dog {
    ...
    /**
     * The default Feature ID. Present in every SuperDog.
     */
     ...
     * <p>
     * If a previously established session context exists, the session will be
     * logged out.
     * @param vendor_code
     *            The Vendor Code.
    ...
    public boolean login(String vendor_code) {
        if (vendor_code == null)
            ...
            synchronized (this) {
                logout();
                System.out.println(
                        "featureid: " + featureid + ", vendor_code: " + vendor_code + ", " + "handle: " + handle);
    ...
    /**
     * Logs in to a Feature according to customizable search parameters. This
     * function is an extended login function, where the search for the Feature
     * can be restricted.
     * <p>
     * If a previously established session context exists, the session will be
     * logged out.
     * @param scope
     *            The dog_scope of the Feature search.
     * @param vendor_code
     *            The Vendor Code.
     * @return true/false - indicates success or failure.
     * @see #login
    ...
    /**
     * Logs out from a session and frees all allocated resources for the
     * session.
     * @return true/false - indicates success or failure.
     * @see #login
     * @see #getLastError
    ...
    /**
     * Encrypts a buffer. This is the reverse operation of the decrypt() method.
     * <p>
     * If the encryption fails (e.g. Dog removed during the process) the data
     * buffer is unmodified.
     * @param buffer
     *            The buffer to be encrypted. (16 bytes minimum).
     * @return true/false - indicates success or failure.
     * @see #decrypt
     * @see #getLastError
     */
    ...
    /**
     * Encrypts a string. This is the reverse operation of the decryptString()
     * method.
     * <p>
     * If the encryption fails (e.g. Dog removed during the process) the data
     * buffer is unmodified.
    ...
    /**
     * Decrypts a buffer. This is the reverse operation of the encrypt() method.
     * <p>
     * If the decryption fails (e.g. Dog removed during the process) the data
     * buffer is unmodified.
     * @param buffer
     *            The buffer to be decrypted. (16 bytes minimum).
     * @return true/false - indicates success or failure.
     * @see #encrypt
     * @see #getLastError
     */
    ...
    /**
     * Decrypts a string. This is the reverse operation of the encryptString()
     * method.
     * <p>
     * If the decryption fails (e.g. Dog removed during the process) the data
     * buffer is unmodified.
    ...
    /**
     * Retrieves information about SuperDog. Acquires information about
     * SuperDog. The programmer can choose the scope and output structure of the
     * data. The function has a "scope" parameter that defines the scope using
     * XML syntax.
     * <p>
     * This function is not used in a login context, so it can be used in a
     * generic "Monitor" application.
     * <p>
     * @param scope
     *            XML definition of the information scope.
     * @param format
     *            XML definition of the output data structure.
     * @param vendor_code
     *            The Vendor Code.
    ...
    /**
     * Writes update information to a SuperDog. The update BLOB contains all
     * necessary data to perform the update: Where to write (to which SuperDog),
     * the necessary access data (Vendor Code) and the update itself.
     * <p>
     * If requested by the update BLOB, the function returns an Acknowledge
     * BLOB, which is signed/encrypted by the updated instance and contains
     * proof that this update was successfully installed.
     * @param update_data
     *            The complete update data.
    ...
    /**
     * Reads the current time from a SuperDog. Time values are returned as the
     * number of seconds that have elapsed since Jan-01-1970 0:00:00 UTC.
     * <p>
