...
public class DogApiVersion {
    ...
    private static native int GetVersion(int major_version[], int minor_version[], int build_server[],
            int build_number[], byte vendor_code[]);
    ...
    public DogApiVersion(String vendor_code) {
        try {
            // Following code is added to ensure that byte array passed to JNI
            // interface
            // is NULL terminated. Ideally the JNI GetVersion interface should
            // be accepting
            // vendor_code as String like other JNI interface dog_login,
            // dog_login_scope etc.
            // But changing JNI interface will result in incompatible function
            // signature
