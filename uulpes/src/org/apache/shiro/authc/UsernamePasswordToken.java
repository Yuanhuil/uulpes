...
/**
 * <p>
 * A simple username/password authentication token to support the most
 * widely-used authentication mechanism. This class also implements the
 * {@link RememberMeAuthenticationToken RememberMeAuthenticationToken} interface
 * to support &quot;Remember Me&quot; services across user sessions as well as
 * the {@link org.apache.shiro.authc.HostAuthenticationToken
 * HostAuthenticationToken} interface to retain the host name or IP address
 * location from where the authentication attempt is occuring.
 * </p>
 * <p/>
 * <p>
 * &quot;Remember Me&quot; authentications are disabled by default, but if the
 * application developer wishes to allow it for a login attempt, all that is
 * necessary is to call {@link #setRememberMe setRememberMe(true)}. If the
 * underlying <tt>SecurityManager</tt> implementation also supports
 * <tt>RememberMe</tt> services, the user's identity will be remembered across
 * sessions.
 * <p/>
 * <p>
 * Note that this class stores a password as a char[] instead of a String (which
 * may seem more logical). This is because Strings are immutable and their
 * internal value cannot be overwritten - meaning even a nulled String instance
 * might be accessible in memory at a later time (e.g. memory dump). This is not
 * good for sensitive information such as passwords. For more information, see
 * the <a href=
 ...
 * <p/>
 * <p>
 * To avoid this possibility of later memory access, the application developer
 * should always call {@link #clear() clear()} after using the token to perform
 * a login attempt.
 * </p>
 *
 * @since 0.1
...
public class UsernamePasswordToken implements HostAuthenticationToken, RememberMeAuthenticationToken {
    ...
    private int dogid;// 加密狗登录id

    private String response;// 加密狗登录随机字符串
     ...
     * <p/>
     * <p>
     * This is a convience constructor and maintains the password internally via
     * a character array, i.e. <tt>password.toCharArray();</tt>. Note that
    ...
    /**
     * Constructs a new UsernamePasswordToken encapsulating the username and
     * password submitted, the inetAddress from where the attempt is occurring,
     * and a default <tt>rememberMe</tt> value of <tt>false</tt>
     *
     * @param username
     *            the username submitted for authentication
     * @param password
    ...
    /**
     * Constructs a new UsernamePasswordToken encapsulating the username and
     * password submitted, as well as if the user wishes their identity to be
    ...
    /**
     * Constructs a new UsernamePasswordToken encapsulating the username and
     * password submitted, if the user wishes their identity to be remembered
     * across sessions, and the inetAddress from where the attempt is ocurring.
     *
     * @param username
     *            the username submitted for authentication
     * @param password
    ...
    public UsernamePasswordToken(final String username, final char[] password, final boolean rememberMe,
            final String host, final String logintype, int dogid, String response) {
    ...
    public UsernamePasswordToken(final String username, final String password, final boolean rememberMe,
        ...
        this(username, password != null ? password.toCharArray() : null, rememberMe, host, logintype, dogid, response);
    }
    /*--------------------------------------------
